...
def qx_escape_domain(d): return d.replace(".", r"\.")
def qx_exact(d):
    return rf"^https?:\/\/{qx_escape_domain(d)}(?::\d+)?(?:\/|$) url reject"
def qx_suffix(d):
    return rf"^https?:\/\/(?:[^.\/]+\.)*{qx_escape_domain(d)}(?::\d+)?(?:\/|$) url reject"
...
def main():
    ...
    qx_rules=(
        [qx_exact(d) for d in sorted(exact_domains)]
        + [qx_suffix(d) for d in sorted(suffix_domains)]
    )
    ...
    (OUT/"quantumultx-rewrite.conf").write_text(
        ...
        "# Use the generated ipv4/ipv6/nftables outputs for fixed-IP HTTPDNS blocking.\n"
